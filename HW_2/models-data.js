...
function generateHat()
{
    ...
    for(var i = 0; i < nRows; i++) {
        for(var j = 0; j < nColumns; j++) {
            var u = -2.0 + 4.0 * j / (nColumns - 1);
            var v = -2.0 + 4.0 * i / (nRows - 1);
            var r = Math.sqrt(u*u + v*v);
            // 墨西哥帽函数: y = (1 - r^2) * exp(-r^2/2)
            var height = (1 - r*r) * Math.exp(-r*r/2);
            data[i][j] = height;
        }
    }
    ...
    for(var i = 0; i < nRows - 1; i++) {
        for(var j = 0; j < nColumns - 1; j++) {
            var x1 = -2.0 + 4.0 * j / (nColumns - 1);
            var z1 = -2.0 + 4.0 * i / (nRows - 1);
            var y1 = data[i][j];
            
            var x2 = -2.0 + 4.0 * (j+1) / (nColumns - 1);
            var z2 = -2.0 + 4.0 * i / (nRows - 1);
            var y2 = data[i][j+1];
            
            var x3 = -2.0 + 4.0 * (j+1) / (nColumns - 1);
            var z3 = -2.0 + 4.0 * (i+1) / (nRows - 1);
            var y3 = data[i+1][j+1];
            
            var x4 = -2.0 + 4.0 * j / (nColumns - 1);
            var z4 = -2.0 + 4.0 * (i+1) / (nRows - 1);
            var y4 = data[i+1][j];
            
            var v1 = vec4(x1, y1, z1, 1.0);
            var v2 = vec4(x2, y2, z2, 1.0);
            var v3 = vec4(x3, y3, z3, 1.0);
            var v4 = vec4(x4, y4, z4, 1.0);
            
            // 根据高度设置颜色渐变
            var colorBase = Math.abs(y1) * 2;
            var color1 = vec4(
                0.5 + colorBase * 0.5,
                0.3 + colorBase * 0.3,
                0.1 + colorBase * 0.4,
            ...
            // 第一个三角形
            points.push(v1); colors.push(color1);
            points.push(v2); colors.push(color2);
            points.push(v3); colors.push(color3);
            
            // 第二个三角形
            points.push(v1); colors.push(color1);
            points.push(v3); colors.push(color3);
            points.push(v4); colors.push(color4);
        }
    }
}
